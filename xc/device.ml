...
let add_device ~xs device backend_list frontend_list private_list ~extra_xenserver_vif_list =
	...
	and extra_xenserver_vif_path = Device_common.extra_xenserver_vif_path_of_device ~xs device in
	...
	Xs.transaction xs (fun t ->
		...
		if (List.length extra_xenserver_vif_list) > 0 then begin
			t.Xst.mkdir extra_xenserver_vif_path;
			t.Xst.setperms extra_xenserver_vif_path (Xenbus_utils.rwperm_for_guest device.frontend.domid);
			List.iter (fun (x, y) ->
				let ip_setting_path = Printf.sprintf "%s/%s" extra_xenserver_vif_path x in
				debug "xenstore-write %s <- %s" ip_setting_path y;
				t.Xst.write ip_setting_path y
			) extra_xenserver_vif_list
		end
	)
	)
...
let add_async ~xs ~hvm x domid =
	...
	*)
	...
	Generic.add_device ~xs device back front priv ~extra_xenserver_vif_list:[];
	device
...
let add (task: Xenops_task.t) ~xs ~devid ~netty ~mac ~carrier ?mtu ?(rate=None) ?(protocol=Protocol_Native) ?(backend_domid=0) ?(other_config=[]) ?(extra_private_keys=[]) ?(extra_xenserver_keys=[]) domid =
	...
	Generic.add_device ~xs device back front extra_private_keys ~extra_xenserver_vif_list:extra_xenserver_keys;
...
let release (task: Xenops_task.t) ~xs (x: device) =
	...
	Hotplug.release task ~xs ~extra_xenserver_path:xenstore_path x
...
let add ~xc ~xs ?(backend_domid=0) ?(protocol=Protocol_Native) domid =
	...
	Generic.add_device ~xs device back front [] ~extra_xenserver_vif_list:[];
	()
