...
let device_by_id xc xs vm kind domain_selection id =
  ...
  | Some frontend_domid ->
    let fetch_all_from_xenstore_f () = (* expensive *)
      let devices = (Device_common.list_frontends ~xs frontend_domid)
        @ (Device_common.list_xenserver_devices ~xs frontend_domid)
      in
      let key = _device_id kind in
      let id_of_device device =
        let path = match kind with
          | Device_common.NetSriovVf -> Device_common.extra_xenserver_path_of_device ~xs device
          | _ -> Device_common.get_private_data_path_of_device device
        in
    ...
    let fetch_one_from_xenstore_f cached_device =
      ...
      let xenstored_device = (Device_common.list_frontends ~xs ~for_devids:[cached_frontend_devid] frontend_domid)
        @ (Device_common.list_xenserver_devices ~xs ~for_devids:[cached_frontend_devid] frontend_domid)
      in
                ...
                | Network.Sriov pci ->
                  let (_: Device_common.device) = 
                    (with_common_params Device.NetSriovVf.add)
                       ...
                       ~extra_xenserver_keys:(static_ip_setting @ xenopsd_backend
                           @ [("mac", mac); id])
                       task frontend_domid
                  in
                  ()
