		...
		with_xc_and_xs
           ...
           let ls_l ~depth root dir =
             ...
             in
              (value_opt, subdirs)
           in
           let rec ls_lR ?(excludes = StringSet.empty) ?(depth = 512) root (quota, acc) dir =
           ...
           let quota, guest_agent =
               ...
               |> List.fold_left
                    (fun acc (dir, excludes, depth) -> ls_lR ?excludes ~depth (Printf.sprintf "/local/domain/%d" di.Xenctrl.domid) acc dir)
                    (quota, [])
           ...
           in
           let path = Device_common.xenops_path_of_domain di.Xenctrl.domid ^ "/guest_agent_quota_reached" in
           (* we don't want the guest controlling how often we warn *)
           let warned_path = Device_common.get_private_path di.Xenctrl.domid ^ "/guest_agent_quota_warned" in
           ( if quota <= 0 then
                try
                  let (_ : string) = xs.Xs.read path in
                  let now = Unix.gettimeofday () in
                  let last = try float_of_string (xs.Xs.read warned_path) with _ -> 0. in
                  if now -. last > float !Xenopsd.vm_guest_agent_xenstore_quota_warn_interval then (
                    (* periodically warn if the quota is still exceeded *)
                    xs.Xs.write warned_path (string_of_float now) ;
                    warn "xenstore guest agent quota is still exceeded for domid %d" di.Xenctrl.domid
                  )
                with _ -> (
                  warn
